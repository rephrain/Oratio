<script>
    ...
    $: patientId = $page.url.searchParams.get("id");
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            if (!patientRes.ok) {
                ...
                addToast(err.error || "Gagal memperbarui data pasien", "error");
                loading = false;
                return;
            }

            addToast(
                `Data pasien ${form.nama_lengkap} berhasil diperbarui`,
                "success",
            );
            goto("/suster");
        } catch (err) {
        ...
        if (patientId) {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    form.village = p.village || "";
                    form.rt = p.rt || "";
                    form.rw = p.rw || "";
                    form.email = p.email || "";
                    form.marital_status = p.marital_status || "S";
                    form.citizenship = p.citizenship || "WNI";
                    form.blood_type = p.blood_type || "";
                    form.rhesus = p.rhesus || "+";
                    form.pregnancy_status = p.pregnancy_status || false;
                    form.tekanan_darah = p.tekanan_darah || "";

                    if (p.handphone) {
                        const parts = p.handphone.split(" ");
                        if (parts.length > 1) {
                            selectedCountryCode = parts[0];
                            inputPhoneNumber = parts.slice(1).join(" ");
                        } else {
                            if (p.handphone.startsWith("+62")) {
                                selectedCountryCode = "+62";
                                inputPhoneNumber = p.handphone.replace(/^\+62/, "");
                            } else {
                                inputPhoneNumber = p.handphone;
                            }
                        }
                    }
                    ...
                    if (p.province) {
                        const matchedProv = $provinces.find(
                            (prov) => prov.name.toLowerCase() === p.province.toLowerCase()
                        );
                        if (matchedProv) {
                            selectedProvinceCode = matchedProv.code;
                            
                            loadingRegency.set(true);
                            try {
                                regencies.set(await fetchRegencies(selectedProvinceCode));
                                if (p.city) {
                                    const matchedReg = $regencies.find(
                                        (reg) => reg.name.toLowerCase() === p.city.toLowerCase()
                                    );
                                    if (matchedReg) {
                                        selectedRegencyCode = matchedReg.code;
                                        ...
                                        try {
                                            ...
                                            if (p.district) {
                                                const matchedDist = $districts.find(
                                                    (dist) => dist.name.toLowerCase() === p.district.toLowerCase()
                                                );
                                                if (matchedDist) {
                                                    selectedDistrictCode = matchedDist.code;
                    ...
                    diseaseHistory = (data.diseases || []).map(d => ({
                        type: d.type || "personal",
                        code: d.code || "",
                        display: d.display || "",
                        system: d.system || "SNOMED",
                    ...
                    addToast("Gagal memuat detail pasien", "error");
                    goto("/suster");
                }
            } catch (err) {
                console.error(err);
            ...
            addToast("ID Pasien tidak ditemukan", "error");
            goto("/suster");
        }
    });
...
<svelte:head>
    <title>Ubah Data Pasien — Oratio Clinic</title>
</svelte:head>
...
<div class="p-8 max-w-5xl mx-auto">
    <div class="mb-8">
        ...
        <p class="text-slate-500 mt-1">
            Perbarui formulir pendaftaran pasien untuk memperbarui data identitas pasien.
        </p>
    </div>
