...
export async function generatePatientProfilePdf(data) {
    ...
    const renderDiseases = (items) => {
        if (items.length === 0) return `<p class="text-xs italic text-slate-500 px-1">No reported records</p>`;
...
// ── Odontogram SVG rendering for PDF ────────────────────────────────────────
...
function buildOdontogramMap(odontogramDetails) {
    ...
    // Now map to the same format as the frontend's mappedOdontogramData
    const result = {};
    for (const tn in grouped) {
        ...
        // Global conditions
        if (d.keadaan === 'mis') result[tn].global = 'Missing';
        else if (['prd', 'fld', 'fud'].includes(d.protesa)) result[tn].global = 'Missing';
        else if (d.keadaan === 'nvt') result[tn].global = 'Non-Vital';
        else if (d.keadaan === 'non') result[tn].global = 'NON';
        else if (d.keadaan === 'une') result[tn].global = 'UNE';
        else if (d.keadaan === 'pre') result[tn].global = 'PRE';
        else if (d.keadaan === 'ano') result[tn].global = 'ANO';
        else if (d.keadaan === 'cfr' || d.keadaan === 'frx') result[tn].global = 'Fracture';
        else if (d.keadaan === 'rrx') result[tn].global = 'Sisa Akar';

        if (d.restorations && d.restorations.length > 0) {
            d.restorations.forEach(r => {
                const mappedData = {
                    condition: d.keadaan, color,
                    restoration: r.restorasi, bahan_restorasi: r.bahan_restorasi,
                    protesa: d.protesa
                };
                if (r.surfaces && r.surfaces.length > 0) {
                    r.surfaces.forEach(s => {
                        const keys = parseSurfaces(s, tn);
                        keys.forEach(k => { if (!result[tn][k]) result[tn][k] = mappedData; });
                    });
                } else {
                    if (!result[tn]['center']) result[tn]['center'] = mappedData;
                }
                if (r.restorasi === 'rct') result[tn].global = 'RCT';
                else if (r.restorasi === 'pon') result[tn].global = 'Missing';
            });
        } else {
...
function renderToothSvg(number, td) {
    ...
    const points = isAnterior
        ? { top: '0,0 100,0 75,35 25,35', right: '100,0 100,100 75,65 75,35', bottom: '0,100 100,100 75,65 25,65', left: '0,0 0,100 25,65 25,35', center: '25,35 75,35 75,65 25,65' }
        : { top: '0,0 100,0 75,25 25,25', right: '100,0 100,100 75,75 75,25', bottom: '0,100 100,100 75,75 25,75', left: '0,0 0,100 25,75 25,25', center: '25,25 75,25 75,75 25,75' };

    const getFill = (surfaceData) => {
        if (surfaceData?.bahan_restorasi === 'amf') return '#000000';
        if (surfaceData?.bahan_restorasi === 'cof') return '#10B981';
        if (surfaceData?.bahan_restorasi === 'fis') return '#FCA5A5';
    ...
    if (cond.includes('.ver')) {
        if (cond === 'M.ver') { if (isRightSideOfMouth) drawArrowRight = true; else drawArrowLeft = true; }
        else if (cond === 'D.ver') { if (isRightSideOfMouth) drawArrowLeft = true; else drawArrowRight = true; }
        else if (['ML.ver', 'DL.ver', 'MP.ver', 'DP.ver', 'L.ver', 'P.ver'].includes(cond)) drawCurvedBelow = true;
        else drawCurvedAbove = true;
    }

    let extras = '';

    // Bridge bracket
    if (isBridgeComponent) extras += `<line x1="-5" y1="-15" x2="105" y2="-15" stroke="#000" stroke-width="6"/>`;

    // FMC / POC crown outline
    if (isFMC || isPOC) extras += `<rect x="0" y="0" width="100" height="100" fill="none" stroke="#000" stroke-width="6"/>`;
    if (isPOC) extras += `<line x1="20" y1="0" x2="20" y2="100" stroke="#000" stroke-width="1.5"/><line x1="40" y1="0" x2="40" y2="100" stroke="#000" stroke-width="1.5"/><line x1="60" y1="0" x2="60" y2="100" stroke="#000" stroke-width="1.5"/><line x1="80" y1="0" x2="80" y2="100" stroke="#000" stroke-width="1.5"/>`;
    ...
    // Non-Vital triangle
    if (globalCondition === 'Non-Vital') extras += `<polygon points="35,100 65,100 50,125" fill="#fff" stroke="#000" stroke-width="2.5" stroke-linejoin="round"/>`;
    // RCT solid triangle
    if (globalCondition === 'RCT') extras += `<polygon points="35,100 65,100 50,125" fill="#000" stroke="#000" stroke-width="2.5" stroke-linejoin="round"/>`;
    // Fracture hash
    if (globalCondition === 'Fracture') extras += `<g stroke="#000" stroke-width="4" stroke-linecap="square"><line x1="20" y1="35" x2="80" y2="35"/><line x1="20" y1="65" x2="80" y2="65"/><line x1="45" y1="20" x2="35" y2="80"/><line x1="65" y1="20" x2="55" y2="80"/></g>`;
    // Sisa Akar V
    if (globalCondition === 'Sisa Akar') extras += `<line x1="15" y1="-10" x2="45" y2="115" stroke="#000" stroke-width="10" stroke-linecap="round"/><line x1="85" y1="-25" x2="45" y2="115" stroke="#000" stroke-width="10" stroke-linecap="round"/>`;
    // Text overlays
    ...
    // Arrows
    if (drawArrowLeft) extras += `<g stroke="#000" stroke-width="4" fill="none" stroke-linecap="round" stroke-linejoin="round"><line x1="80" y1="-15" x2="20" y2="-15"/><polyline points="35,-25 20,-15 35,-5"/></g>`;
    if (drawArrowRight) extras += `<g stroke="#000" stroke-width="4" fill="none" stroke-linecap="round" stroke-linejoin="round"><line x1="20" y1="-15" x2="80" y2="-15"/><polyline points="65,-25 80,-15 65,-5"/></g>`;
    if (drawCurvedAbove) extras += `<g stroke="#000" stroke-width="4" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M 30,-5 Q 50,-35 80,-10"/><polyline points="65,-15 80,-10 75,-25"/></g>`;
    if (drawCurvedBelow) extras += `<g stroke="#000" stroke-width="4" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M 30,105 Q 50,135 80,110"/><polyline points="65,115 80,110 75,125"/></g>`;
