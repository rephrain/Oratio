...
import { encounters, patients, users, encounterPrescriptions, encounterReferrals, encounterItems, items, terminologyMaster, encounterOdontograms, odontogramTeeth, odontogramSurfaces, odontogramRestorations, odontogramRestorationSurfaces, odontogramDiagnoses, odontogramProcedures } from '$lib/server/db/schema.js';
import { alias } from 'drizzle-orm/pg-core';
import { eq } from 'drizzle-orm';
import { generateSoapFormPdf, generateSoapWhoFormPdf } from '$lib/server/pdfGenerator.js';

/**
 * GET /api/encounters/:id/pdf
 * Generates and serves a PDF version of the clinical SOAP encounter document.
 */
export async function GET({ params, url }) {
	...
	try {
		...
		const prescriptions = await db.select({
			...
			.where(eq(encounterPrescriptions.encounter_id, id));
		...
		const eItems = await db.select({
			...
			.where(eq(encounterItems.encounter_id, id));
			...
			if (odontograms.length > 0) {
				...
				for (const tooth of teeth) {
					...
					const diagnoses = await db.select({
						id: odontogramDiagnoses.id, icd10_id: odontogramDiagnoses.icd10_id, is_primary: odontogramDiagnoses.is_primary, icd10_code: icd10Term.code, icd10_display: icd10Term.display
					...
					const procedures = await db.select({
						id: odontogramProcedures.id, icd9cm_id: odontogramProcedures.icd9cm_id, icd9cm_code: icd9cmTerm.code, icd9cm_display: icd9cmTerm.display
					...
					if (surfaces.length > 0) {
						for (const surf of surfaces) {
							...
							odontogramDetails.push({
								id: surf.id, odontogram_id: odontoId, tooth_number: tooth.tooth_number, surface: surf.surface,
								keadaan: tooth.keadaan, restorasi: restData.restorasi || null, bahan_restorasi: restData.bahan_restorasi || null,
								protesa: tooth.protesa, bahan_protesa: tooth.bahan_protesa,
								icd10_id: primaryDiag?.icd10_id || null, is_primary: primaryDiag?.is_primary || false, icd10_code: primaryDiag?.icd10_code || null, icd10_display: primaryDiag?.icd10_display || null,
								icd9cm_id: primaryProc?.icd9cm_id || null, icd9cm_code: primaryProc?.icd9cm_code || null, icd9cm_display: primaryProc?.icd9cm_display || null,
								created_at: tooth.created_at, all_diagnoses: diagnoses, all_procedures: procedures
							});
						}
					} else {
						const firstRest = restorations[0] || null;
						odontogramDetails.push({
							id: tooth.id, odontogram_id: odontoId, tooth_number: tooth.tooth_number, surface: '',
							keadaan: tooth.keadaan, restorasi: firstRest?.restorasi || null, bahan_restorasi: firstRest?.bahan_restorasi || null,
							protesa: tooth.protesa, bahan_protesa: tooth.bahan_protesa,
							icd10_id: primaryDiag?.icd10_id || null, is_primary: primaryDiag?.is_primary || false, icd10_code: primaryDiag?.icd10_code || null, icd10_display: primaryDiag?.icd10_display || null,
							icd9cm_id: primaryProc?.icd9cm_id || null, icd9cm_code: primaryProc?.icd9cm_code || null, icd9cm_display: primaryProc?.icd9cm_display || null,
							created_at: tooth.created_at, all_diagnoses: diagnoses, all_procedures: procedures
		...
		if (encounter.form_mode === 'SOAP_WHO') {
			pdfBuffer = await generateSoapWhoFormPdf({ 
				encounter, patient, doctor, prescriptions, referrals, items: eItems, origin, rxText: encounter.resep,
				odontograms, odontogramDetails
			});
		} else {
