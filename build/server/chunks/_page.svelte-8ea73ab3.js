import { c as create_ssr_component, v as validate_component, g as add_attribute } from './ssr-4a5a9ccc.js';
import { a as getJakartaDateString, b as getJakartaMonthString } from './formatters-f4f9a52a.js';
import { R as RichSelect } from './RichSelect-b89df93a.js';
import { E as ENCOUNTER_STATUSES } from './constants-d1611a67.js';
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = `${$$result.head += `<!-- HEAD_svelte-a5jv8f_START -->${$$result.title = `<title>Encounter History — Oratio Clinic</title>`, ""}<!-- HEAD_svelte-a5jv8f_END -->`, ""} <div class="-m-6 flex h-[calc(100vh-73px)] bg-slate-50 overflow-hidden font-sans relative"><div class="flex-1 flex flex-col min-h-0 p-6"><div class="flex items-center justify-between mb-6 shrink-0" data-svelte-h="svelte-8tw9qf"><div><h2 class="text-xl font-bold text-blue-900">Encounter History</h2> <p class="text-sm text-slate-500">View and filter previous patient visits</p></div></div>  <div class="bg-white p-4 rounded-t-xl border-x border-t border-slate-200 flex flex-wrap items-center gap-4 shrink-0 shadow-sm"><div class="flex items-center gap-2"><div class="w-40">${validate_component(RichSelect, "RichSelect").$$render(
      ...
      {}
    )}</div></div> <div class="flex items-center gap-2"><label class="text-xs font-bold text-slate-500 uppercase tracking-wider" data-svelte-h="svelte-14sc0dp">Status:</label> <div class="w-44">${validate_component(RichSelect, "RichSelect").$$render(
      $$result,
      {
