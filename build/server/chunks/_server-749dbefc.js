...
import { d as db, C as doctorSuster, e as encounters, j as statusHistory } from './index3-af609ec3.js';
...
async function GET({ locals, url }) {
  ...
  const assignedDoctors = await db.select({ doctor_id: doctorSuster.doctor_id }).from(doctorSuster).where(eq(doctorSuster.suster_id, susterId));
  ...
  if (doctorIds.length === 0) {
    return json({
      ...
      avgWaitMinutes: 0,
      avgWaitMinutesChange: 0,
      avgTreatmentMinutes: 0,
      avgTreatmentMinutesChange: 0
    });
  }
...
export { GET };
//# sourceMappingURL=_server-749dbefc.js.map
