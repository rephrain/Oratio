...
function fmtBpStatus(bp) {
	...
	const [sys] = parts;
	if (sys < 120) return { label: 'NORMAL', bg: 'bg-tertiary', text: 'text-on-tertiary' };
	if (sys < 130) return { label: 'ELEVATED', bg: 'bg-amber-500', text: 'text-white' };
	if (sys < 140) return { label: 'HIGH', bg: 'bg-orange-500', text: 'text-white' };
...
export async function generatePatientProfilePdf(data) {
	...
	const renderDiseases = (items) => {
		if (items.length === 0) return `<p class="text-sm italic text-on-surface-variant">No records</p>`;
