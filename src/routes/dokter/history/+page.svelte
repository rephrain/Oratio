<script>
	...
	$: sortedTableEncounters = [...filteredTableEncounters].sort((a, b) => {
		...
		if (sortKey === "id") {
			...
			valA = a.encounter?.patient_id || "";
			valB = b.encounter?.patient_id || "";
		} else if (sortKey === "doctor") {
			valA = a.doctor_name || "";
			valB = b.doctor_name || "";
		} else if (sortKey === "reason") {
			valA = a.encounter_reason_display || "";
			valB = b.encounter_reason_display || "";
...
>
	<div class="flex-1 flex flex-col min-h-0 p-6">
		<div class="flex items-center justify-between mb-6 shrink-0">
			<div>
				...
				<p class="text-sm text-slate-500">
					View and filter previous patient visits
				</p>
			</div>
		</div>
		...
		<!-- Filters Bar -->
		<div
			class="bg-white p-4 rounded-t-xl border-x border-t border-slate-200 flex flex-wrap items-center gap-4 shrink-0 shadow-sm"
		>
			...
			<div class="flex items-center gap-2">
				<label
					class="text-xs font-bold text-slate-500 uppercase tracking-wider ml-2"
					>Doctor:</label
				>
				<div class="w-64">
					<RichSelect
						...
						on:select={(e) => {
							tableDoctorFilter = e.detail.value;
							loadEncounters();
						}}
			...
			<div class="flex items-center gap-2">
				...
				<div class="w-44">
					<RichSelect
						...
						on:select={(e) => {
							tableStatusFilter = e.detail.value;
							loadEncounters();
						}}
			...
			{:else}
					...
					<table class="w-full text-left text-sm whitespace-nowrap">
						...
						<tbody class="divide-y divide-slate-100">
							{#each paginatedEncounters as row}
								...
								<tr
									class="hover:bg-slate-50/50 transition-colors cursor-pointer {expandedRowId ===
									row.encounter?.id
										? 'bg-primary/5'
										: ''}"
									on:click={() =>
										toggleRow(row.encounter?.id)}
								>
									<td
										class="px-6 py-4 font-bold text-slate-900 font-mono text-xs"
									>
										{row.encounter?.id}
									</td>
									<td
									...
									<td
										...
										title={row.encounter_reason_display}
									>
									...
									<td class="px-6 py-4">
										...
										{:else if status === "Completed"}
											<span
												class="px-3 py-1 bg-emerald-100 text-emerald-700 rounded-full text-[11px] font-bold uppercase tracking-wider"
												>Completed</span
											>
										{:else}
											<span
									...
									<tr class="bg-slate-50/80">
										...
										>
											...
											>
												<div class="space-y-4">
													<div>
														...
														<p
															class="text-sm text-slate-700 whitespace-pre-wrap leading-relaxed"
														>
															{row.encounter
																?.subjective ||
																"-"}
												...
												<div
													class="space-y-4 bg-white/50 p-4 rounded-xl border border-slate-100"
												>
													<div>
														<h4
															class="text-[10px] font-bold text-primary uppercase tracking-widest mb-1"
														>
															Prescription (Resep)
														</h4>
														<p
															class="text-sm text-slate-700 font-medium whitespace-pre-wrap leading-relaxed"
														>
															{row.encounter
																?.resep || "-"}
														</p>
													...
													<div>
														...
														<p
															class="text-sm text-slate-600 italic whitespace-pre-wrap leading-relaxed"
														>
															{row.encounter
																?.keterangan ||
																"-"}
											...
											{#if row.clinical_photos && row.clinical_photos.length > 0}
												...
												>
													<h4
														class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-1"
													>
														<span
													...
													>
														{#each row.clinical_photos as photo}
															<a
																...
																target="_blank"
																class="block w-28 h-28 rounded-xl border border-slate-200 overflow-hidden hover:border-primary transition-all hover:shadow-md hover:-translate-y-0.5 group relative bg-white"
															>
																{#if photo.mime_type?.startsWith("image/")}
																	...
																	>
																		<span
																			class="material-symbols-outlined text-white shadow-sm"
																			>open_in_new</span
																		>
																	</div>
																{:else}
																	<div
																		class="w-full h-full flex flex-col items-center justify-center text-slate-400 p-2 text-center group-hover:text-primary transition-colors"
																	>
																		<span
																			class="material-symbols-outlined mb-1 text-2xl group-hover:scale-110 transition-transform"
																			>description</span
																		>
																		<span
																			class="text-[10px] truncate w-full font-medium"
																			title={photo.file_name}
																			>{photo.file_name}</span
																		>
																	</div>
																{/if}
															</a>
				...
				>
					<div class="text-xs text-slate-500 font-medium">
						Menampilkan <strong class="text-slate-900"
							>{filteredTableEncounters.length === 0
								? 0
								: (currentPage - 1) * itemsPerPage + 1}</strong
						>
						-
						<strong class="text-slate-900"
							>{Math.min(
								currentPage * itemsPerPage,
								filteredTableEncounters.length,
							)}</strong
						>
						dari
						<strong class="text-slate-900"
							>{filteredTableEncounters.length}</strong
						> kunjungan
					</div>
					<div class="flex items-center gap-2">
						<button
						...
						<div
							class="text-xs font-bold text-slate-700 w-8 text-center"
						>
							{currentPage}
						</div>
						<button
