<script>
	...
	import { formatElapsedTime, formatTime, getShiftCountdown } from '$lib/utils/formatters.js';
	...
	async function loadShifts() {
		...
		try {
			...
			const resp = await res.json();
			doctorShifts = (resp.data || []).filter(s => s.doctor_id === user.id);
			updateShift();
		} catch {}
	}
	...
	const tableColumns = [
		{ key: 'queue', label: '#', format: (_, r) => r.encounter?.queue_number || '-', sortable: true },
		{ key: 'patient', label: 'Pasien', format: (_, r) => r.patient_name || '-' },
		{ key: 'status', label: 'Status', format: (_, r) => r.encounter?.status || '-' },
		{ key: 'time', label: 'Waktu', format: (_, r) => formatTime(r.encounter?.created_at) },
		{ key: 'dur', label: 'Durasi', format: (_, r) => formatElapsedTime(r.encounter?.created_at) }
	];
...
<svelte:head>
	<title>Dashboard Dokter — Oratio Dental</title>
</svelte:head>
