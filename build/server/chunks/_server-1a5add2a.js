...
import { d as db } from './index3-41fb71fd.js';
...
async function GET({ url, locals }) {
  ...
  const dateFilter = dateFrom && dateTo ? sql`AND DATE(e.created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Jakarta') >= ${dateFrom}
		       AND DATE(e.created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Jakarta') <= ${dateTo}` : sql``;
  ...
  let prevBaseWhere = sql`WHERE 1=0`;
  if (dateFrom && dateTo) {
    const df = /* @__PURE__ */ new Date(dateFrom + "T00:00:00Z");
    const dt = /* @__PURE__ */ new Date(dateTo + "T00:00:00Z");
...
export { GET };
//# sourceMappingURL=_server-1a5add2a.js.map
