...
import { payments, encounters, patients, users, encounterItems, items } from '$lib/server/db/schema.js';
import { eq } from 'drizzle-orm';
import { alias } from 'drizzle-orm/pg-core';
import { generatePaymentReceiptPdf } from '$lib/server/pdfGenerator.js';
...
export async function GET({ params, url }) {
    ...
    .leftJoin(doctors, eq(payments.doctor_id, doctors.id))
    ...
    .where(eq(encounterItems.encounter_id, payment.encounter_id));
    ...
    const pdfBuffer = await generatePaymentReceiptPdf({
        payment, encounter, patient, doctor, cashier, items: eItems, origin
    });
