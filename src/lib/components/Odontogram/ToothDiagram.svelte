<script>
	...
	$: if (readyToFlash) {
		if (previousHash !== null && dataHash !== previousHash) {
			flash = true;
	...
	function getStroke(surfaceData, defaultStroke = "#94a3b8") {
		if (
			surfaceData?.condition === "car" ||
			surfaceData?.condition === "cav"
		)
			return "#000000";
	...
	$: isBridgeComponent = ["meb", "pob", "pon", "abu", "Bridge"].includes(
		center?.restoration,
	);
	$: isDenture = ["prd", "fld", "fud", "pld", "pud", "hld", "hud"].includes(
		center?.protesa,
	);
	...
	$: isRightSideOfMouth = ["1", "4", "5", "8"].includes(
	...
	$: if (cond.includes(".ver")) {
		if (cond === "M.ver") {
			if (isRightSideOfMouth) drawArrowRight = true;
			else drawArrowLeft = true;
		} else if (cond === "D.ver") {
			if (isRightSideOfMouth) drawArrowLeft = true;
			else drawArrowRight = true;
		} else if (
			["ML.ver", "DL.ver", "MP.ver", "DP.ver"].includes(cond) ||
			cond === "L.ver" ||
			cond === "P.ver"
		) {
...
<div
	class="flex flex-col items-center gap-1.5 transition-transform duration-200 hover:-translate-y-0.5 hover:scale-105 relative {flash
		? 'animate-jump-flash'
		: ''}"
>
	<div
		class="text-[13px] font-bold text-slate-500 leading-none pb-0.5 pointer-events-none select-none"
	>
		{number}
	</div>

	<div
		class="relative cursor-pointer filter drop-shadow-[0_1px_2px_rgba(0,0,0,0.05)] hover:drop-shadow-[0_3px_5px_rgba(0,0,0,0.1)] transition-all duration-200"
	>
		<svg
			width="42"
			height="42"
		...
		>
			...
			{#if ["NON", "UNE", "PRE", "ANO"].includes(globalCondition)}
				...
				<text
					...
					paint-order="stroke"
					text-anchor="middle"
					class="pointer-events-none">{globalCondition}</text
				>
			{/if}
			{#if isIPX}
				...
				<text
					...
					text-anchor="middle"
					class="pointer-events-none"
					dominant-baseline="hanging">IPX</text
				>
			{/if}
			{#if isDenture}
				...
				<text
					...
					text-anchor="middle"
					class="pointer-events-none"
					dominant-baseline="hanging">PRD/FLD</text
				>
			{/if}
			{#if drawArrowLeft}
				<g
...
<style>
	@keyframes jumpFlash {
		...
		25% {
			...
			filter: drop-shadow(0 8px 12px rgba(16, 185, 129, 0.4));
		}
		50% {
	...
	.animate-jump-flash {
		animation: jumpFlash 0.8s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
		z-index: 20;
	}
</style>
