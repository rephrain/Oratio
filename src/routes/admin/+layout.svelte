...
<div
	class="relative flex min-h-screen w-full overflow-x-hidden bg-background-light dark:bg-background-dark font-display text-slate-900 dark:text-slate-100"
>
	<!-- Sidebar -->
	<aside
		class="w-64 bg-slate-sidebar text-slate-400 hidden lg:flex flex-col border-r border-slate-800/50"
	>
		...
		<nav class="flex-1 px-4 space-y-1 custom-scrollbar overflow-y-auto">
			<div
				class="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] px-3 py-4"
			>
				Main Menu
			</div>
			<a
				class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all {currentPath ===
				'/admin'
			...
			>
				...
				<span
					class="text-sm {currentPath === '/admin'
						? 'font-semibold'
			...
			>
				...
				<span
					class="text-sm {currentPath.startsWith('/admin/patients')
						? 'font-semibold'
						: 'font-medium'}">Patients</span
				>
			</a>
			<a
				class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all {currentPath.startsWith(
			...
			>
				...
				<span
					class="text-sm {currentPath.startsWith('/admin/users')
						? 'font-semibold'
						: 'font-medium'}">Users / Staff</span
				>
			</a>
			<a
				class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all {currentPath.startsWith(
			...
			>
				...
				<span
					class="text-sm {currentPath.startsWith('/admin/encounters')
						? 'font-semibold'
						: 'font-medium'}">Encounters</span
				>
			</a>
			<a
				class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all {currentPath.startsWith(
			...
			>
				...
				<span
					class="text-sm {currentPath.startsWith('/admin/payments')
						? 'font-semibold'
						: 'font-medium'}">Payments</span
				>
			</a>
			<a
				class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all {currentPath.startsWith(
			...
			>
				...
				<span
					class="text-sm {currentPath.startsWith('/admin/items')
						? 'font-semibold'
						: 'font-medium'}">Items & DB</span
				>
			</a>
			<a
				class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all {currentPath.startsWith(
			...
			>
				...
				<span
					class="text-sm {currentPath.startsWith('/admin/terminology')
						? 'font-semibold'
						: 'font-medium'}">Terminology</span
				>
			</a>
			<a
				class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all {currentPath.startsWith(
			...
			>
				...
				<span
					class="text-sm {currentPath.startsWith('/admin/import')
						? 'font-semibold'
						: 'font-medium'}">Data Import</span
				>
			</a>
		</nav>
	</aside>
	...
	<main class="flex-1 flex flex-col min-w-0 overflow-hidden">
		<!-- Top Nav -->
		<header
			class="h-16 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800/50 flex items-center justify-between px-8 sticky top-0 z-20"
		>
			...
			<div class="flex items-center gap-6">
				<div class="flex items-center gap-4">
					<button
						...
						id="admin-notification-toggle-btn"
					>
				...
				<div class="relative">
					...
					>
						<div class="text-right hidden sm:block">
							...
							<p
								class="text-[10px] text-slate-500 mt-1 uppercase tracking-wider"
							>
								{user?.role || "System Admin"}
							</p>
						</div>
						<div
							class="size-10 rounded-full bg-primary/20 flex items-center justify-center text-primary font-bold uppercase overflow-hidden border border-primary/20"
						>
					...
					{#if showProfileMenu}
						<div
							class="absolute right-0 mt-3 w-48 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-100 dark:border-slate-800 py-2 z-[60] origin-top-right transition-all"
						>
							<div
								class="px-4 py-2 border-b border-slate-100 dark:border-slate-800 mb-1 sm:hidden"
							>
								<p
								...
								<p
									class="text-[10px] text-slate-500 uppercase tracking-wider truncate"
								>
									{user?.role || "System Admin"}
								</p>
							</div>
							...
							></div>
							<button
								class="w-full flex items-center gap-3 px-4 py-2 hover:bg-rose-50 dark:hover:bg-rose-500/10 text-rose-500 transition-colors"
								on:click={logout}
							>
