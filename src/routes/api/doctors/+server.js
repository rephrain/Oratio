...
import { users, doctorShifts } from '$lib/server/db/schema.js';
...
export async function GET({ url }) {
	...
	if (dayOfWeek !== null && dayOfWeek !== undefined) {
		const shifts = await db.select().from(doctorShifts)
			.where(eq(doctorShifts.day_of_week, parseInt(dayOfWeek)));
